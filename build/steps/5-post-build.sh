...
if [ "$AWS_EC2_NODE_NO" = "" ] || [ "$AWS_EC2_NODE_NO" = "0" ] ; then
  ...
  $workspace/build/steps/post-build/install-aws-cli.sh || { echo "Executing $workspace/build/steps/post-build/install-aws-cli.sh" ; exit 1; }
  $workspace/build/steps/post-build/cronjobs.sh || { echo "Executing $workspace/build/steps/post-build/cronjobs.sh failed" ; exit 1; }
  $workspace/build/steps/post-build/update-ami.sh || { echo "Executing $workspace/build/steps/post-build/update-ami.sh" ; exit 1; }
fi