...
class Client extends AbstractHasDispatcher implements ClientInterface
{
    ...
    public function duplicates($searchTerm){

        $array = array();
        $array[] = new \SoapVar('<ns1:sObjects xsi:type="urn1:Account"><Name>'.$searchTerm.'</Name></ns1:sObjects>', XSD_ANYXML, 'urn1:Account', null, 'sObjects' );

        try {
            $soapResponse =  $this->call(
                'findDuplicates',
                    $array
            );
        ...
        if(isset($soapResponse[0]->duplicateResults[0]->matchResults)){
            $response['count'] = count($soapResponse[0]->duplicateResults[0]->matchResults);
            $response['matches'] = $soapResponse[0]->duplicateResults[0]->matchResults;
        }
