...
namespace Drupal\een_event_import\Plugin\migrate_plus\data_parser;
 ...
 * @DataParser(
 *   id = "eventbrite_json",
 *   title = @Translation("Eventbrite JSON")
 * )
 */
class EventbriteJson extends DataParserPluginBase implements ContainerFactoryPluginInterface {
  ...
  protected function getSourceData($url) {
    ...
    foreach ($selectors as $selector) {
      $final_data = $source_data[$selector];
    }
    // Check for multipage response
    if((isset($source_data['pagination']['page_count']) && $source_data['pagination']['page_count'] > 1)) {
      $num_pages = $source_data['pagination']['page_count'];
      for($i = 2; $i <= $num_pages; $i++) {
        $new_url = $url . '&page=' . $i;
        $response = $this->getDataFetcherPlugin()->getResponseContent($new_url);
        ...
        $selectors = explode('/', trim($this->itemSelector, '/'));
        foreach ($selectors as $selector) {
          $final_data = array_merge($final_data, $source_data[$selector]);
        }
