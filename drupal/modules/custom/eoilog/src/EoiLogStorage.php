...
namespace Drupal\eoilog;

/**
 * Class BulkEmailStorageBulkEmailStorage.
 */
class  EoiLogStorage {
  /**
  ...
  public static function update(array $entry) {
    try {
      ...
      $count = db_update('eoilog')
        ->fields($entry)
        ->condition('token', $entry['token'])
        ->execute();
    }
    catch (\Exception $e) {
  ...
  public static function load(array $entry = []) {
    ...
    $select = db_select('eoilog');
    $select->fields('eoilog');
